...
use support::{
    active_rebase_head_name, append_to_file, commit_file, dgr, dgr_ok, git_ok, git_stdout,
...
fn restacks_tracked_descendants_after_commit_and_preserves_store_files() {
    with_temp_repo("dgr-commit-cli", |repo| {
        ...
        let events_before = fs::read_to_string(repo.join(".git/.dagger/events.ndjson")).unwrap();

        append_to_file(repo, "auth.txt", "follow-up\n");
        git_ok(repo, &["add", "auth.txt"]);

        let output = dgr_ok(repo, &["commit", "--message", "feat: auth follow-up"]);
...
fn restacks_child_after_amend_using_old_head_oid() {
    with_temp_repo("dgr-commit-cli", |repo| {
        ...
        let output = dgr_ok(repo, &["commit", "--amend", "--no-edit"]);
        let stdout = strip_ansi(&String::from_utf8(output.stdout).unwrap());
